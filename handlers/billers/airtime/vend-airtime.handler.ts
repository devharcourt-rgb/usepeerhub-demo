...
async function vendAirtimeHandler(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  const { userId } = getUser(req);
  const { amount, phoneNumber, network } = req.body as {
    amount: number;
    phoneNumber: string;
  ...
  try {
    ...
    if (isFraudulent) {
      ...
      await ActivityLogModel.create({
        ...
        metadata: { reason: "fraud_detection", context: "airtime" },
      });
    ...
    // Created PENDING — Nomba's airtime purchase is asynchronous, so this
    // only moves to COMPLETED/FAILED once the webhook confirms the outcome.
    const transaction = await TransactionModel.create({
      user: userId,
      amount: amountInKobo,
      type: TransactionType.DEBIT,
      description: TransactionDescription.AIRTIME,
      status: TransactionStatus.PENDING,
      currency: nigerianNaira._id,
      metadata: { phoneNumber: localPhoneNumber, network },
    });

    const merchantTxRef = `airtime-${transaction._id.toString()}`;

    try {
      const nombaResponse = await airtimeService.purchaseAirtime({
...
export default vendAirtimeHandler;
