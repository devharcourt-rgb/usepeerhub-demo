...
async function validateCustomerDetailHandler(
  req: Request,
  res: Response,
  next: NextFunction
) {
  const { billerCode, itemCode, customer } = req.body;
  ...
  try {
    ...
    const flwClientResponse = await flwClient.validateCustomerDetails({
      billerCode,
      itemCode,
      customer,
    });
...
export default validateCustomerDetailHandler;
