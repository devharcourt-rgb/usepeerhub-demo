...
async function vendCableHandler(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  ...
  try {
    ...
    if (isFraudulent) {
      ...
      await ActivityLogModel.create({
        ...
        metadata: { reason: "fraud_detection", context: "cable" },
      });
    ...
    // Created PENDING — Nomba's cable subscription is asynchronous, so this
    ...
    const transaction = await TransactionModel.create({
      ...
      metadata: { cableTvType, customerId, customerName, code, product },
    });

    const merchantTxRef = `cable-${transaction._id.toString()}`;

    try {
      const nombaResponse = await cableService.subscribe({
        cableTvType,
        customerId,
    ...
    return res.status(HTTPStatus.ACCEPTED).json({
      message: "Cable subscription is processing",
      data: transaction,
    });
  } catch (error) {
    next(error);
...
export default vendCableHandler;
